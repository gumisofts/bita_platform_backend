...
from django.db.models import Q, F, Count, Avg, Case, When, Value, CharField
...
from inventories.models import ItemVariant, Item, Property
from business.models import Category, Industry, Business
...
class MarketplaceProductViewSet(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Property filtering
        properties = self.request.query_params.get('properties')  # Format: "color:red,size:large"
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter(name='categories', type={'type': 'array', 'items': {'type': 'string'}}, 
                           description='Filter by category IDs'),
            OpenApiParameter(name='industries', type={'type': 'array', 'items': {'type': 'string'}}, 
                           description='Filter by industry IDs'),
            OpenApiParameter(name='businesses', type={'type': 'array', 'items': {'type': 'string'}}, 
                           description='Filter by business IDs'),
            OpenApiParameter(name='business_types', type={'type': 'array', 'items': {'type': 'string'}}, 
                           description='Filter by business types'),
            OpenApiParameter(name='min_price', type=OpenApiTypes.DECIMAL, 
                           description='Minimum price filter'),
            OpenApiParameter(name='max_price', type=OpenApiTypes.DECIMAL, 
                           description='Maximum price filter'),
            OpenApiParameter(name='in_stock_only', type=OpenApiTypes.BOOL, 
                           description='Show only in-stock items'),
            OpenApiParameter(name='min_quantity', type=OpenApiTypes.INT, 
                           description='Minimum quantity filter'),
            OpenApiParameter(name='verified_only', type=OpenApiTypes.BOOL, 
                           description='Show only verified businesses'),
            OpenApiParameter(name='properties', type=OpenApiTypes.STR, 
            ...
            OpenApiParameter(name='expire_date_from', type=OpenApiTypes.DATE, 
                           description='Filter by expiry date from'),
            OpenApiParameter(name='expire_date_to', type=OpenApiTypes.DATE, 
    ...
    def featured(self, request):
        ...
        queryset = self.get_queryset().filter(
            item__business__is_verified=True,
            quantity__gt=0
    ...
    def similar(self, request, pk=None):
        ...
        similar_queryset = self.get_queryset().filter(
            item__categories__in=product.item.categories.all()
    ...
    def advanced_search(self, request):
        ...
        if data.get('query'):
            search_vector = SearchVector('name', 'item__name', 'item__description', 'sku')
...
class MarketplaceCategoryViewSet(ReadOnlyModelViewSet):
    ...
    def tree(self, request):
        ...
        ).prefetch_related('category_set').distinct()
...
class MarketplaceBusinessViewSet(ReadOnlyModelViewSet):
    ...
    queryset = Business.objects.filter(
        is_active=True
    ).select_related('address').prefetch_related('categories')
    ...
    def products(self, request, pk=None):
        ...
        products = ItemVariant.objects.filter(
            item__business=business,
            is_visible_online=True
        ).select_related('item').prefetch_related('properties')
