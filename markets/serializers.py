...
from inventories.models import Item, ItemVariant, ItemImage, Property, Pricing
...
class MarketplaceCategorySerializer(serializers.ModelSerializer):
    ...
    def get_item_count(self, obj):
        return obj.items.filter(
            variants__is_visible_online=True,
...
class MarketplaceItemVariantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ItemVariant
        fields = [
            'id', 'name', 'selling_price', 'quantity', 'sku', 'batch_number',
            'expire_date', 'man_date', 'is_default', 'is_returnable',
            'is_visible_online', 'receive_online_orders', 'notify_below',
            'created_at', 'updated_at',
            # Item fields
            'item_name', 'item_description', 'item_images', 'inventory_unit',
            # Business
            'business',
            # Categories
            'categories',
            ...
            'pricings',
            # Calculated fields
            'is_in_stock', 'stock_status', 'discount_percentage', 'average_rating',
            'is_available_online'
        ]
    ...
    def get_is_available_online(self, obj):
        return (
            obj.is_visible_online and 
            obj.receive_online_orders and 
            obj.quantity > 0 and
            obj.item.business.is_active
        )
...
class MarketplaceItemVariantListSerializer(MarketplaceItemVariantSerializer):
    ...
    class Meta(MarketplaceItemVariantSerializer.Meta):
        fields = [
            'id', 'name', 'selling_price', 'quantity', 'sku',
            'item_name', 'item_description', 'inventory_unit',
            'business', 'categories', 'item_images',
            'is_in_stock', 'stock_status', 'discount_percentage',
            'is_available_online', 'created_at'
        ]
