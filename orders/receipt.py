...
def generate_order_receipt(order) -> bytes:
    ...
    if address:
        addr_parts = [p for p in [address.sublocality, address.locality, address.admin_1, address.country] if p]
    ...
    for item in items:
        ...
        display_name = item_name if item_name == variant_name else f"{item_name} — {variant_name}"
        rows.append([
    ...
    totals_data = [
        ...
        [Paragraph(f"VAT ({int(VAT_RATE * 100)}%)", normal), Paragraph(f"{vat_amount:,.2f} ETB", right_align)],
        [Paragraph("<b>Total Payable</b>", normal), Paragraph(f"<b>{total_payable:,.2f} ETB</b>", right_align)],
    ]
    ...
    if is_returned:
        doc.build(story, onFirstPage=_draw_returned_watermark, onLaterPages=_draw_returned_watermark)
    else:
        doc.build(story)
