...
class ConversationParticipantInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['user', 'employee', 'role', 'joined_at', 'last_read_at', 'is_muted', 'is_active']
...
class MessageInline(admin.TabularInline):
    model = Message
    extra = 0
    fields = ['sender', 'content_preview', 'message_type', 'created_at', 'is_edited', 'is_deleted']
...
class ConversationAdmin(admin.ModelAdmin):
    list_display = [
        'id', 'title_display', 'conversation_type', 'business_link',
        'participants_count', 'messages_count', 'created_by_link',
        'last_message_at', 'is_active', 'created_at'
    ]
    list_filter = [
        'conversation_type', 'is_active', 'business',
        'created_at', 'last_message_at'
    ]
    search_fields = ['title', 'business__name', 'created_by__first_name', 'created_by__last_name']
    ...
    fieldsets = (
        (None, {
            'fields': ('id', 'title', 'conversation_type', 'business', 'created_by', 'is_active')
        }),
        ('Timestamps', {
...
class ConversationParticipantAdmin(admin.ModelAdmin):
    list_display = [
        'id', 'conversation_link', 'user_link', 'employee_link',
        'role', 'business_name', 'joined_at', 'last_read_status',
        'is_muted', 'is_active'
    ]
    list_filter = [
        'role', 'is_muted', 'is_active', 'joined_at',
        'conversation__business', 'conversation__conversation_type'
    ]
    search_fields = [
        'user__first_name', 'user__last_name', 'user__email',
        'conversation__title', 'employee__business__name'
    ]
    ordering = ['-joined_at']
    ...
    fieldsets = (
        (None, {
            'fields': ('id', 'conversation', 'user', 'employee', 'role')
        }),
        ('Activity', {
    ...
    def conversation_link(self, obj):
        ...
        title = obj.conversation.title or f"{obj.conversation.get_conversation_type_display()}"
    ...
    def last_read_status(self, obj):
        if obj.last_read_at:
            ...
            if time_diff.days > 0:
                ...
                return format_html(
                    '<span style="color: green;">Recently</span>'
                )
...
class MessageAdmin(admin.ModelAdmin):
    list_display = [
        'id', 'conversation_link', 'sender_link', 'content_preview',
        'message_type', 'reply_status', 'status_indicators',
    ...
    search_fields = [
        ...
        'conversation__title', 'conversation__business__name'
    ]
    ordering = ['-created_at']
    readonly_fields = [
        'id', 'created_at', 'updated_at', 'edited_at', 'deleted_at'
    ]
    raw_id_fields = ['conversation', 'sender', 'sender_employee', 'reply_to', 'attachment']
    
    fieldsets = (
        (None, {
            'fields': ('id', 'conversation', 'sender', 'sender_employee', 'content', 'message_type')
        }),
        ('Attachments & Replies', {
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'conversation', 'sender', 'sender_employee', 'reply_to'
    ...
    def reply_status(self, obj):
        if obj.reply_to:
            return format_html(
                '<span style="color: blue;">↳ Reply to message</span>'
            )
        replies_count = obj.replies.count()
        if replies_count > 0:
            return format_html(
                '<span style="color: green;">{} replies</span>',
                replies_count
            )
        return '-'
...
class MessageStatusAdmin(admin.ModelAdmin):
    list_display = [
        'id', 'message_link', 'participant_link', 'status',
        'business_name', 'status_changed_at'
    ]
    list_filter = ['status', 'status_changed_at', 'participant__conversation__business']
    search_fields = [
        'message__content', 'participant__user__first_name',
        'participant__user__last_name', 'participant__conversation__title'
    ]
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'message', 'participant', 'participant__user',
            'participant__conversation', 'participant__conversation__business'
        )
    ...
    def participant_link(self, obj):
        url = reverse('admin:chat_conversationparticipant_change', args=[obj.participant.id])
        name = f"{obj.participant.user.first_name} {obj.participant.user.last_name}".strip()
        return format_html('<a href="{}">{}</a>', url, name or obj.participant.user.email)
...
class ConversationInvitationAdmin(admin.ModelAdmin):
    list_display = [
        'id', 'conversation_link', 'invited_by_link', 'invited_user_link',
        'status', 'business_name', 'created_at', 'responded_at'
    ]
    ...
    fieldsets = (
        (None, {
            'fields': ('id', 'conversation', 'invited_by', 'invited_user', 'invited_employee')
        }),
        ('Invitation Details', {
            'fields': ('status', 'message', 'expires_at')
        }),
        ('Timestamps', {
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'conversation', 'invited_by', 'invited_user',
            'invited_employee', 'conversation__business'
        )
