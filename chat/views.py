...
class ConversationViewSet(ModelViewSet):
    ...
    def add_participants(self, request, pk=None):
        ...
        return Response({
            "message": f"Added {len(new_participants)} participants"
        })
...
class MessageViewSet(CreateModelMixin, ListModelMixin, RetrieveModelMixin, GenericViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Message.objects.filter(
            ...
            'sender', 'sender_employee', 'conversation', 'reply_to'
        ).prefetch_related(
            'statuses__participant__user'
        ).order_by('-created_at')
...
class ConversationInvitationViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        return ConversationInvitation.objects.filter(
    ...
    @extend_schema(
        ...
        description="Accept a conversation invitation"
    )
    @action(detail=True, methods=['post'])
    ...
    @extend_schema(
        ...
        description="Decline a conversation invitation"
    )
    @action(detail=True, methods=['post'])
