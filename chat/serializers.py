...
class ConversationParticipantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'user', 'employee', 'role', 'joined_at', 
            'last_read_at', 'is_muted', 'is_active'
        ]
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Message
        fields = [
            'id', 'conversation', 'sender', 'sender_employee', 'content',
            'message_type', 'attachment', 'reply_to', 'replies_count',
            'is_edited', 'edited_at', 'is_deleted', 'deleted_at',
            'created_at', 'updated_at', 'statuses'
        ]
        read_only_fields = [
            'id', 'sender', 'sender_employee', 'is_edited', 'edited_at',
    ...
    def get_reply_to(self, obj):
        if obj.reply_to:
            return {
                ...
                'sender': obj.reply_to.sender.first_name + ' ' + obj.reply_to.sender.last_name,
...
class ConversationListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Conversation
        fields = [
            'id', 'title', 'conversation_type', 'business', 'created_by',
            'is_active', 'last_message_at', 'created_at', 'participants_count',
    ...
    def get_last_message(self, obj):
        ...
        if last_message:
            return {
                ...
                'sender_name': last_message.sender.first_name + ' ' + last_message.sender.last_name,
...
class ConversationDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'title', 'conversation_type', 'business', 'created_by',
            'created_by_name', 'is_active', 'last_message_at', 'created_at',
...
class ConversationCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_participant_user_ids(self, value):
        if len(value) < 1:
            raise serializers.ValidationError({"detail":["At least one participant is required."]})
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            for user_id in participant_user_ids:
                ...
                role = 'admin' if (user_id == user.id and 
                                 conversation.conversation_type == 'group') else 'member'
...
class ConversationInvitationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'conversation', 'conversation_title', 'invited_by',
            'invited_by_name', 'invited_user', 'invited_user_name',
            'invited_employee', 'status', 'message', 'expires_at',
    ...
    def get_conversation_title(self, obj):
        return obj.conversation.title or f"{obj.conversation.get_conversation_type_display()}"
...
class MarkAsReadSerializer(serializers.Serializer):
    ...
    mark_all = serializers.BooleanField(
        default=False,
        help_text="Mark all messages in conversation as read"
    )
